	...
	override function create()
	{
		...
		for (i in 0...12) {
			...
			switch (i) {
				...
				case 11:
					icon = new CreditIcon(836, 371, 'Sisox', 'https://github.com/Sirox228/Psych-Engine-Android.git');
					text = new FlxSprite(963, 390).loadGraphic(Paths.image('credits/text/creditSirox'));
				case 12:
					icon = new CreditIcon(834, 225, 'Sikus', 'https://github.com/stikiPorter/source-code-port-rosie-week2.git');
					text = new FlxSprite(965, 258).loadGraphic(Paths.image('credits/text/creditStiki'));
			}
			icon.x += FlxG.width * 3;
			text.x += FlxG.width * 3;
			icon.antialiasing = true;
